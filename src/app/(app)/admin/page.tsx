...
const COLORS = ["#4F46E5", "#10B981", "#F59E0B", "#EF4444", "#6366F1", "#E879F9"];
...
const currentOrders = filterState ? filteredOrders : orders;
  ...
  const paymentCounts = currentOrders.reduce<Record<string, number>>((acc, order) => {
    acc[order.paymentState] = (acc[order.paymentState] || 0) + 1;
    return acc;
  }, {});

  const userOrders = currentOrders.reduce<Record<string, { name: string; count: number }>>(
    (acc, order) => {
    ...
    لا توجد نتائج لهذا الفلتر
            ...
            <div className="sm:hidden mt-8">
              {currentOrders.map((order) => (
                <div key={order._id} className="bg-white shadow rounded-xl p-4 mb-4">
                  ...
                  <div className="text-sm text-gray-700 mb-2">
                    {typeof order.productId === "object" && order.productId?.title
                      ? order.productId.title
                      : "منتج غير معروف"}
                  </div>
                  <div className="flex items-center gap-2 mb-2">
                    {typeof order.productId === "object" && order.productId?.images?.[0] && (
                      <Image
...
>
  ✏️ تعديل الحالة
            ...
            {showModal && (
              <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
                <div className="bg-white rounded-lg p-6 w-80 text-black">
                  <h4 className="text-lg font-semibold mb-4">اختر حالة الطلب</h4>
                  <select
                  ...
                  <div className="flex justify-end gap-3 flex-wrap">
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded text-white ${
                        filterState ? "bg-indigo-600 hover:bg-indigo-700" : "bg-gray-400 cursor-not-allowed"
                      }`}
                    >
                      تأكيد
                    </button>
                ...
                <tbody className="divide-y">
                  {currentOrders.map((order, index) => (
                    <tr key={order._id}>
                      ...
                      <td className="p-3">{order.userId?.email || "لا يوجد"}</td>
                      <td className="p-3">{order.userId?.phoneNumber || "لا يوجد"}</td>
                      <td className="p-3">{typeof order.productId === "object" && order.productId?.title ? order.productId.title : "بدون منتج"}</td>
                      <td className="p-3">
                        {typeof order.productId === "object" && order.productId?.images?.[0] ? (
                          <Image src={order.productId.images[0]} alt="Product" className="h-10 w-10 object-cover rounded" width={40} height={40} unoptimized />
                        ) : "—"}
                      </td>
                      <td className="p-3">{typeof order.productId === "object" && order.productId?.price ? `${order.productId.price} EGP` : "—"}</td>
            ...
            {editModalOpen && selectedOrderId && (
              <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
                <div className="bg-white rounded-lg p-6 w-80 text-black">
                  <h4 className="text-lg font-semibold mb-4">تعديل حالة الطلب</h4>
                  <select
                    className="w-full border rounded p-2 mb-4"
                    value={newStatus}
                            ...
                            setOrders((prev) =>
                              prev.map((order) =>
                                order._id === selectedOrderId
                                  ? { ...order, status: newStatus as Orderreport["status"] }
                                  : order
                              )
                            );
                          } else {
